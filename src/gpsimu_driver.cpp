...
namespace sensor
{
    ...
    int jyGpsImu::run(void) {
        char rec_buff[100];
        int rec_num = sp_motor_.readSerialPort(rec_buff,100,0);
        ParseData(rec_buff,rec_num);
    }
    ...
    void jyGpsImu::ParseData(const char *chr, int length) {
        for(int i=0;i<length;++i) {
            rec_buf_.push_back(chr[i]);
        }
        
        while (rec_buf_.size() >= 11) {
            if (!((rec_buf_[0] == 0x55) & ((rec_buf_[1] & 0x50)==0x50)))
            {
                rec_buf_.pop_front();
                continue;
            }
            ...
            int sum =0;
            for(int i=0;i<10;++i) {
                data_buffer[i] = rec_buf_[i];
                sum += data_buffer[i];
            }
            
            // if(rec_buf_[10]== sum &0xff)
                DecodeData(data_buffer);
    ...
    void jyGpsImu::DecodeData(const char chrBuf[])
    {
